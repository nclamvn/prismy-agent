...
# Initialize logger
logger = logging.getLogger('app_ultra_modern')
...
import os
import tempfile
from chunking.text_chunker import SmartTextChunkerEnhanced, ChunkResult
from chunking.layout_chunker import LayoutAwareChunker, LayoutChunk
...
def get_real_openai_translation_chunked_fixed(text, target_lang, api_key, model="gpt-3.5-turbo", progress_callback=None):
    """Enhanced version with SmartTextChunkerEnhanced and improved context handling"""
    ...
    try:
        ...
        chunker = SmartTextChunkerEnhanced(
            max_chunk_size=2800,  # Optimized size for GPT models
            overlap_size=150  # Increased overlap for better context
        )
        formula_processor = FormulaProcessor()

        # Process short text directly
        if len(text) < 2500:  # Increased threshold for better handling
            client = openai.OpenAI(api_key=api_key)
            
            lang_names = {
                'vi': 'Vietnamese', 'en': 'English', 'zh': 'Chinese (Simplified)',
                'zh-tw': 'Chinese (Traditional)', 'ja': 'Japanese', 'fr': 'French', 
                'de': 'German', 'es': 'Spanish', 'ko': 'Korean', 'it': 'Italian',
        ...
        # Long text - use enhanced chunking with formula handling
        if progress_callback:
            progress_callback(5, "🔄 Initializing enhanced chunking system...")

        # Pre-process formulas
        formulas = formula_processor.detect_formulas(text)
        if formulas and progress_callback:
            progress_callback(8, f"📐 Detected {len(formulas)} formulas - preserving them")
            # TODO: Add formula preservation logic

        # Use SmartTextChunkerEnhanced for chunking
        chunks = chunker.chunk_text(text)
        total_chunks = len(chunks)
        
        if progress_callback:
            progress_callback(10, f"📊 Split into {total_chunks} chunks with context preservation")
        
        logger.info(f"🔍 TRANSLATION DEBUG: Processing {total_chunks} chunks with enhanced context")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                # Update progress
                progress_percentage = 10 + (i / total_chunks) * 80
                if progress_callback:
                    progress_callback(progress_percentage, f"🔄 Translating chunk {i+1}/{total_chunks}")
                
                # Build context-aware prompt
                system_prompt = f"""You are a professional translator specialized in accurate translations.

{f'''PREVIOUS CONTEXT (for reference only, DO NOT translate this part):
{chunk.context}

''' if chunk.metadata.has_context else ''}Please translate the following content to {target_language}, ensuring:
1. Preserve all formatting and structure
2. Maintain technical terminology accuracy
3. Ensure smooth continuation from context
4. Keep any special characters or markers intact

Content to translate:"""

                text_to_translate = chunk.main_content
                
                logger.info(f"🔄 Processing chunk {i+1}: {len(text_to_translate)} chars")
                ...
                try:
                    ...
                    logger.info(f"✅ Chunk {i+1} completed: {len(translated_text)} chars translated")
                    
                except openai.APIError as api_err:
                    logger.error(f"OpenAI API error in chunk {i+1}: {str(api_err)}")
                    raise
                
                # Adaptive rate limiting based on response time
                time.sleep(max(0.1, min(1.0, chunk_time * 0.2)))
                
            except Exception as e:
                logger.error(f"❌ Error in chunk {i+1}: {str(e)}")
                # Preserve original content on error with detailed error info
                translated_chunks.append({
                    'chunk_id': chunk.chunk_id,
                    'translated': f"[TRANSLATION ERROR - PRESERVED CONTENT]: {chunk.main_content[:500]}...",
        ...
        if progress_callback:
            progress_callback(95, "🔧 Combining results with context preservation...")
        
        # Intelligent chunk combination with overlap handling
        combined_result = combine_chunks_safely(translated_chunks, total_chunks)
        
        if progress_callback:
            progress_callback(100, "✅ Translation completed successfully!")
        
        # Calculate final metrics
        total_time = time.time() - start_time
        chars_per_second = len(text) / total_time if total_time > 0 else 0
        avg_chunk_time = sum(c['time'] for c in chunk_times) / len(chunk_times) if chunk_times else 0
        
        logger.info(f"🎉 FINAL RESULT: {len(combined_result)} chars from {successful_chunks}/{total_chunks} chunks")
        
        return {
            'translated_text': combined_result,
            'source_lang': 'auto-detected',
            'target_lang': target_language,
            'model_used': model,
            'confidence': 0.95 if successful_chunks == total_chunks else 0.7,
            ...
            'final_length': len(combined_result),
            'success': True
        }
        ...
        return {
            'translated_text': f"❌ Translation system error: {str(e)}\n\nOriginal text preserved:\n{text[:2000]}...",
            'success': False,
            'error': str(e),
            'error_type': type(e).__name__,
            'error_details': {
...
from chunking.chunk_combiner import combine_chunks_safely


def clean_combined_text(text: str) -> str:
    """Clean up combined text by removing excessive whitespace and formatting.
    ...
    # Normalize multiple spaces to double space
...
# FUNCTION UPDATE CHO PIPELINE
def real_text_processing_pipeline_fixed_no_data_loss(text, config):
    """Pipeline xử lý text ĐÃ SỬA - Không mất dữ liệu"""
    ...
    try:
        ...
        # Thông báo chunking nếu cần
        will_use_chunking = len(text) > 2500
        if will_use_chunking:
            st.info(f"📊 Long text detected ({char_count:,} chars) - Using safe chunking system")
        
        # Progress callback
        def update_progress(percentage, message):
            with progress_container.container():
                st.progress(percentage / 100, message)
            with status_container.container():
                st.info(message)
        
        # Stage 1: Analysis (10%)
        update_progress(10, "🧠 Analyzing document...")
        
        # Detect language
        detected_lang = 'en'  # Default
        try:
            # Vietnamese detection
            if any(char in text for char in ['ă', 'â', 'đ', 'ê', 'ô', 'ơ', 'ư']):
                detected_lang = 'vi'
            # Chinese detection
            elif any('\u4e00' <= char <= '\u9fff' for char in text[:200]):
                detected_lang = 'zh'
            # Japanese detection  
            elif any('\u3040' <= char <= '\u309f' or '\u30a0' <= char <= '\u30ff' for char in text[:200]):
                detected_lang = 'ja'
        except:
            detected_lang = 'en'
        ...
        update_progress(20, f"📄 Preparing for translation...")
        time.sleep(0.5)
        ...
        if openai_key:
            update_progress(30, "🤖 Starting AI translation (with safe chunking)...")
            
            # Sử dụng function đã sửa
            translation_result = get_real_openai_translation_chunked_fixed(
                text, 
                target_lang_code, 
                openai_key,
                config.get('ai_model', 'gpt-3.5-turbo'),
                progress_callback=update_progress
            )
        else:
            translation_result = {
                'translated_text': f"[DEMO MODE - Add OpenAI API key for real translation]\n\nOriginal text ({char_count:,} chars):\n{text[:1000]}...",
                'success': True,
                'confidence': 0.7,
                'model_used': 'demo',
                'chunks_processed': 1
            }
            update_progress(100, "✅ Demo processing complete")
        ...
        # Show success info
        if translation_result.get('chunks_processed', 0) > 1:
            st.success(f"✅ Successfully processed {translation_result['chunks_processed']} chunks!")
            
            # Show comparison
            col1, col2 = st.columns(2)
            with col1:
                st.metric("📝 Original Length", f"{char_count:,} chars")
            with col2:
                st.metric("🌐 Translated Length", f"{len(translated_text):,} chars")
            
            # Show tokens if available
            if translation_result.get('tokens_used'):
                st.info(f"🔢 Total tokens used: {translation_result['tokens_used']:,}")
        ...
        metadata = {
            ...
            'config': config,
            'chunks_used': translation_result.get('chunks_processed', 0),
            'translation_length': len(translated_text)
        }
        
        # Display results
        display_real_text_results(text, translated_text, metadata)
        
    except Exception as e:
        progress_container.empty()
        status_container.empty()
        st.error(f"❌ Processing failed: {str(e)}")
        st.info("💡 Please check your API key and input text")


# THAY THẾ CALL TRONG MAIN FUNCTION
# Từ: real_text_processing_pipeline_fixed_with_chunking(input_text, real_processing_config)
# Thành: real_text_processing_pipeline_fixed_no_data_loss(input_text, real_processing_config)

from chunking.text_chunker import SmartTextChunkerEnhanced as SmartTextChunker

class ChunkedTranslationEngine:
    """Engine xử lý dịch thuật với chunk processing"""
    ...
    def translate_large_text(self, text: str, target_lang: str, 
                           progress_callback=None) -> Dict[str, Any]:
        """Dịch văn bản lớn với chunk processing"""
        ...
        try:
            ...
            if progress_callback:
                progress_callback(10, f"Đã chia thành {total_chunks} chunks")
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    if progress_callback:
                        progress_callback(progress, f"Đang dịch chunk {i+1}/{total_chunks}")
                    
                    # Dịch chunk
                    result = self._translate_single_chunk(chunk, target_lang)
            ...
            if progress_callback:
                progress_callback(95, "Đang ghép kết quả...")
            
            # Ghép chunks đã dịch
            combined_translation = self._combine_translated_chunks(translated_chunks)
            ...
            return {
                ...
                'success_rate': len(translated_chunks) / total_chunks * 100,
                'details': {
                    'successful': translated_chunks,
            ...
            return {
                'success': False,
                'error': str(e),
                'translated_text': f"Lỗi chunking: {str(e)}"
            }
    
    def _translate_single_chunk(self, chunk: ChunkResult, target_lang: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if chunk.metadata.has_context and chunk.context:
                system_prompt = f"""You are a professional translator.
                
                CONTEXT (for reference only, DO NOT translate this part):
                {chunk.context.strip()}
                
                Please translate ONLY the following main content to {target_lang}. Ensure smooth continuation from the context above:"""
                text_to_translate = chunk.main_content
            else:
                system_prompt = f"You are a professional translator. Translate the following text to {target_lang}, preserving formatting and meaning:"
                text_to_translate = chunk.main_content
    ...
    def _combine_translated_chunks(self, translated_chunks: List[Dict]) -> str:
        ...
        for chunk in sorted_chunks:
            translated = chunk['translated'].strip()
            
            # Loại bỏ duplicate content ở overlap
            if combined_text and len(combined_text) > 100:
                # Tìm overlap và loại bỏ
                last_100 = combined_text[-100:].strip()
                first_100 = translated[:100].strip()
                
                # Tìm overlap đơn giản
                for i in range(50, 0, -1):
                    if last_100[-i:] in first_100:
                        # Tìm thấy overlap, bỏ phần trùng
                        overlap_pos = first_100.find(last_100[-i:])
                        if overlap_pos >= 0:
                            translated = translated[overlap_pos + i:].strip()
                        break
            
            combined_text += "\n" + translated if combined_text else translated
        
        return combined_text.strip()

# Sử dụng trong Streamlit
def create_chunked_translation_system(api_key: str, model: str = "gpt-3.5-turbo"):
    """Tạo hệ thống dịch thuật với chunking"""
    return ChunkedTranslationEngine(api_key, model)

# Test functions
    """Test hệ thống chunking"""
    
    # Sample long text
    long_text = """
    Đây là một văn bản dài để test hệ thống chunking. 
    ...
    chunker = SmartTextChunker(max_chunk_size=500, overlap_size=50)
    ...
    for i, chunk in enumerate(chunks):
        ...
        print(f"  Words: {len(chunk.content.split())} words")
...
def get_api_cost(model, token_count):
    """Calculate estimated API cost based on model and token count"""
    costs = {
        'gpt-4': (0.03, 0.06),  # Input: $0.03, Output: $0.06 per 1K tokens
        'gpt-3.5-turbo': (0.0015, 0.002),  # Input: $0.0015, Output: $0.002 per 1K tokens
        'claude-3-opus': (0.015, 0.075),  # Input: $0.015, Output: $0.075 per 1K tokens
        'claude-3-sonnet': (0.003, 0.015),  # Input: $0.003, Output: $0.015 per 1K tokens
        'claude-2': (0.008, 0.024),  # Input: $0.008, Output: $0.024 per 1K tokens
    }
    
    if model in costs:
        input_cost, output_cost = costs[model]
        # Assume output tokens are similar to input tokens
        total_cost = (input_cost * token_count / 1000) + (output_cost * token_count / 1000)
        return total_cost
    return 0

def estimate_translation_time(model, token_count):
    """Estimate translation time based on model and token count"""
    tokens_per_second = {
        'gpt-4': 15,
        'gpt-3.5-turbo': 30,
        'claude-3-opus': 20,
    ...
    base_rate = tokens_per_second.get(model, 25)
    estimated_seconds = token_count / base_rate
    
    # Add overhead for API calls and processing
    overhead = 2.0 if token_count < 1000 else 4.0
...
def main():
    ...
    if 'translation_start_time' not in st.session_state:
        st.session_state.translation_start_time = None
    if 'translation_end_time' not in st.session_state:
        st.session_state.translation_end_time = None
    ...
    with st.sidebar:
        ...
        # Update text content from current input or file
        if 'text_input' in st.session_state and st.session_state.text_input:
            st.session_state.text_content = st.session_state.text_input
        elif 'file_uploader' in st.session_state and st.session_state.file_uploader is not None:
            st.session_state.text_content = st.session_state.file_uploader
        ...
        if st.session_state.text_content:
            ...
            estimated_tokens = estimate_tokens(st.session_state.text_content)
            estimated_cost = get_api_cost(ai_model, estimated_tokens)
            estimated_time = estimate_translation_time(ai_model, estimated_tokens)
            ...
            if word_count > 0:
                st.info(f"ℹ️ Translation with {ai_model} will take approximately {int(estimated_time)} seconds")

            # Show actual translation time if available
            if st.session_state.translation_end_time and st.session_state.translation_start_time:
                actual_time = st.session_state.translation_end_time - st.session_state.translation_start_time
                st.success(f"✅ Actual translation time: {actual_time:.1f} seconds")
...
def document_processing_tab(config):
    """UI for the Document Processing tab"""
    import streamlit as st
...
✨ Ready for the ultimate processing experience!""",
    ...
    with col2:
        if st.button("🚀 Start Ultimate Processing", type="primary", use_container_width=True):
            if input_text.strip():
                # Record start time
                st.session_state.translation_start_time = time.time()
                ...
                # Record end time
                st.session_state.translation_end_time = time.time()
                
                # Display translation time
                translation_time = st.session_state.translation_end_time - st.session_state.translation_start_time
...
def display_real_text_results(original_text, translated_text, metadata):
    """Display REAL text processing results with enhanced timing metrics"""
    ...
    try:
        ...
        st.info("💡 Advanced export features not available")
    ...
    with col2:
        ...
        if metadata.get('chunks_used', 0) > 1:
            st.metric(
                label="⏱️ Avg Chunk Time",
                value=f"{metadata.get('avg_chunk_time', 0):.1f}s",
                delta="Per Chunk"
            )
    
    with col3:
        if metadata.get('chars_per_second', 0) > 0:
            st.metric(
                label="🚀 Speed",
                value=f"{metadata.get('chars_per_second', 0):.0f}",
                delta="Chars/Second"
            )
        st.metric(
            label="🌍 Language", 
    ...
    # Show chunk timing details if available
    if metadata.get('chunk_times'):
        st.markdown("### ⏱️ Chunk Processing Details")
        chunk_data = pd.DataFrame(metadata['chunk_times'])
        
        # Create timing visualization
        fig = px.bar(chunk_data, 
                    x='chunk_id', 
                    y='time',
                    title='Processing Time per Chunk',
                    labels={'chunk_id': 'Chunk', 'time': 'Time (seconds)'})
    ...
    with col2:
        if docx_exporter:
            try:
                ...
                # Export to DOCX with safe metadata access
                docx_exporter.export_translation_result(
                    original_text=original_text,
                    translated_text=translated_text,
                    metadata={
                        'detected_lang': metadata.get('detected_lang', 'unknown'),
                        'target_lang': metadata.get('config', {}).get('target_lang', 'unknown'),
                ...
                st.download_button(
                    label="📋 Download DOCX",
                    data=docx_data,
                    file_name=f"translation_{metadata['config']['target_lang']}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    use_container_width=True
                )
    ...
    with col3:
        if pdf_exporter:
            try:
                ...
                # Reset PDF exporter story
                pdf_exporter.story = []
                
                # Export to PDF with safe metadata access
                pdf_exporter.export_translation_result(
                    original_text=original_text,
                    translated_text=translated_text,
                    metadata={
                        'detected_lang': metadata.get('detected_lang', 'unknown'),
                ...
                st.download_button(
                    label="📑 Download PDF",
                    data=pdf_data,
                    file_name=f"translation_{metadata['config']['target_lang']}.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
    ...
    with col4:
        try:
            ...
            if st.button("☁️ Save to Drive", use_container_width=True):
                try:
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.txt') as tmp_txt:
                        tmp_txt.write(translated_text.encode('utf-8'))
                        files_to_upload.append(('TXT', tmp_txt.name))
                    
                    # DOCX file if available
                    if docx_exporter:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.docx') as tmp_docx:
                            ...
                            files_to_upload.append(('DOCX', tmp_docx.name))
                    ...
                    if pdf_exporter:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_pdf:
                            ...
                            files_to_upload.append(('PDF', tmp_pdf.name))
                    
                    # Create folder in Drive with safe target language
                    target_lang = metadata.get('config', {}).get('target_lang', 'unknown')
                    folder_name = f"Translation_{target_lang}_{int(time.time())}"
                    folder = drive.create_folder(folder_name)
                    
                    # Upload files
                    uploaded_files = []
                    for file_type, file_path in files_to_upload:
                        result = drive.upload_file(file_path, folder['id'])
                        uploaded_files.append(f"{file_type}: {result['link']}")
                        os.unlink(file_path)
            ...
            st.info("Google Drive integration not available")
...
def get_real_openai_translation_chunked_fixed(text, target_lang, api_key, model="gpt-3.5-turbo", progress_callback=None):
    """PHIÊN BẢN MỚI với chunking system - Thay thế get_real_openai_translation"""
    ...
    try:
        ...
        if len(text) < 3000:  # Text ngắn, xử lý thường
            client = openai.OpenAI(api_key=api_key)
            ...
            return {
                ...
                'success': True,
                'processing_time': processing_time,
                'chunk_times': [],  # Single chunk, no individual timing
                'avg_chunk_time': processing_time
            }
        
        # Text dài, sử dụng chunking
        if progress_callback:
            progress_callback(5, "🔄 Preparing chunking system...")
        
        # Initialize chunker from imported module
        from chunking.text_chunker import SmartTextChunker
        chunker = SmartTextChunker(max_chunk_size=3000, overlap_size=200)
        chunks = chunker.chunk_text(text)
        total_chunks = len(chunks)
        
        if progress_callback:
            progress_callback(10, f"📊 Chia thành {total_chunks} chunks")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                progress = 10 + (i / total_chunks) * 80
                if progress_callback:
                    progress_callback(progress, f"🔄 Dịch chunk {i+1}/{total_chunks}")
                
                # Xử lý context
                if chunk['has_context'] and "[CONTEXT_SEPARATOR]" in chunk['text']:
                    context_part, main_part = chunk['text'].split("[CONTEXT_SEPARATOR]", 1)
                    system_prompt = f"""Bạn là chuyên gia dịch thuật chuyên nghiệp.

CONTEXT (để hiểu ngữ cảnh, KHÔNG dịch phần này):
{context_part.strip()}

Hãy dịch phần chính sau đây sang {target_language}, giữ nguyên định dạng và ý nghĩa:"""
                    text_to_translate = main_part.strip()
                else:
                    system_prompt = f"Bạn là chuyên gia dịch thuật. Dịch văn bản sau sang {target_language}, giữ nguyên định dạng và ý nghĩa:"
                ...
                print(f"⚠️ Lỗi chunk {i+1}: {str(e)}")
                translated_chunks.append({
                    'chunk_id': chunk['chunk_id'],
                    'translated': f"[LỖI CHUNK {i+1}]: {str(e)}"
                })
        
        if progress_callback:
            progress_callback(95, "🔧 Ghép kết quả...")
        ...
        total_time = time.time() - start_time
        avg_chunk_time = sum(c['time'] for c in chunk_times) / len(chunk_times) if chunk_times else total_time
...
def display_real_pdf_results(filename, results):
    ...
    with col2:
        if docx_exporter and results['translation']:
            try:
                # Create temp DOCX file
                import tempfile
                import os
                ...
                # Export to DOCX with safe access
                docx_exporter.export_content(
                    content={
                        'title': f"PDF Processing Results - {filename}",
                        'text': full_text,
                        'translation': results.get('translation', {}).get('translated_text', ''),
                ...
                st.download_button(
                    label="📋 Download DOCX",
                    data=docx_data,
                    file_name=f"{filename}_processed.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    use_container_width=True
                )
    ...
    with col3:
        if pdf_exporter and results['translation']:
            try:
                # Create temp PDF file
                import tempfile
                import os
                ...
                st.download_button(
                    label="📑 Download PDF",
                    data=pdf_data,
                    file_name=f"{filename}_processed.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
        ...
        try:
            ...
            if st.button("☁️ Save to Drive", use_container_width=True):
                try:
                    ...
                    # TXT file
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.txt') as tmp_txt:
                        tmp_txt.write(full_text.encode('utf-8'))
                        files_to_upload.append(('TXT', tmp_txt.name))
                    
                    # Translation TXT if available
                    if results['translation']:
                        translated_text = results['translation'].get('translated_text', '')
                        with tempfile.NamedTemporaryFile(delete=False, suffix='_translation.txt') as tmp_trans:
                            tmp_trans.write(translated_text.encode('utf-8'))
                            files_to_upload.append(('Translation', tmp_trans.name))
                    ...
                    # Create folder in Drive with safe filename
                    safe_filename = "".join(c for c in filename if c.isalnum() or c in (' ', '-', '_')).strip()
                    folder_name = f"PDF_Processing_{safe_filename}_{int(time.time())}"
                    folder = drive.create_folder(folder_name)
    ...
    with col2:
        if st.button("🚀 Start REAL AI Processing", type="primary", use_container_width=True):
            if input_text.strip():
                ...
                real_text_processing_pipeline_fixed_no_data_loss(input_text, real_processing_config)
                
            else:
                st.warning("⚠️ Please enter some text to process")

# SỬA LẠI FUNCTION real_text_processing_pipeline_fixed
def real_text_processing_pipeline_fixed_no_data_loss(text, config):
    """PHIÊN BẢN MỚI với chunking system - Thay thế real_text_processing_pipeline_fixed"""
    
    progress_container = st.empty()
    status_container = st.empty()
    
    try:
        # Use session state for timing if available
        if hasattr(st.session_state, 'translation_start_time'):
            st.session_state.translation_start_time = time.time()
        start_time = time.time()
        word_count = len(text.split())
        
        # Hiển thị thông tin chunking nếu text dài
        if len(text) > 3000:
            st.info(f"📊 Text dài ({len(text)} ký tự) - Sẽ sử dụng chunking system")
        ...
        update_progress(10, "🧠 Detecting language...")
        ...
        update_progress(20, "📄 Analyzing document structure...")
        doc_type = config.get('doc_type', 'general')
        time.sleep(0.5)
        
        # Stage 3: AI Translation với Chunking
        openai_key = config.get('openai_key', '').strip()
        target_lang_code = config.get('target_lang_code', 'vi')
        
        if openai_key:
            update_progress(30, "🤖 Starting chunked AI translation...")
            translation_result = get_real_openai_translation_chunked_fixed(
                text, 
                target_lang_code, 
                openai_key,
        ...
        # Display chunking info if used
        if translation_result.get('chunks_processed'):
            st.success(f"✅ Xử lý thành công {translation_result['chunks_processed']} chunks!")
            st.info(f"📊 Total tokens used: {translation_result.get('tokens_used', 0)}")
