...
# THAY THẾ HOÀN TOÀN class SmartTextChunker và các function liên quan

class SmartTextChunkerFixed:
    """Hệ thống chia nhỏ văn bản KHÔNG MẤT DỮ LIỆU - Phiên bản đã sửa"""
    
    def __init__(self, max_chunk_size=2800, overlap_size=150):
        self.max_chunk_size = max_chunk_size  # Giảm size để an toàn
        self.overlap_size = overlap_size      # Giảm overlap để tránh confusion
        
        # Patterns ưu tiên chia theo thứ tự
        self.split_patterns = [
    ...
    def chunk_text(self, text: str) -> List[Dict[str, Any]]:
        """Chia văn bản KHÔNG MẤT DỮ LIỆU"""
        ...
        print(f"🔪 CHUNKING DEBUG: Text length = {len(text)} chars")
        
        while current_pos < len(text):
            # Xác định vùng chunk hiện tại
            ...
            if chunk_end < len(text):
                ...
                optimal_cut = len(text)
            
            # Lấy nội dung chunk CHÍNH (không có overlap)
            main_content = text[current_pos:optimal_cut]
            
            # Thêm context minimal cho chunk sau đầu tiên
            if chunk_id > 0 and self.overlap_size > 0:
                # Lấy context từ phía trước (nhưng đánh dấu rõ ràng)
                context_start = max(0, current_pos - self.overlap_size)
                context_content = text[context_start:current_pos]
                
                # Tạo chunk với context được đánh dấu rõ ràng
                full_chunk_text = f"[CONTEXT_START]\n{context_content}\n[CONTEXT_END]\n\n[MAIN_CONTENT_START]\n{main_content}\n[MAIN_CONTENT_END]"
                has_context = True
            else:
            ...
            chunk_info = {
                ...
                'main_content': main_content,  # Lưu riêng nội dung chính
                ...
                'original_end': optimal_cut
            }
            
            chunks.append(chunk_info)
            
            print(f"   📝 Chunk {chunk_id}: pos {current_pos}-{optimal_cut} ({optimal_cut-current_pos} chars)")
            ...
            if optimal_cut == current_pos:
                print(f"⚠️ WARNING: No progress at pos {current_pos}, breaking...")
                break
    ...
    def _find_optimal_cut_point(self, text: str, start: int, max_end: int) -> int:
        """Tìm điểm cắt tối ưu KHÔNG LÀM MẤT DỮ LIỆU"""
        
        if max_end >= len(text):
            return len(text)
        
        # Tìm trong vùng cuối chunk (400 chars cuối để có nhiều lựa chọn hơn)
        ...
        for pattern in self.split_patterns:
            ...
            if matches:
                # Lấy match cuối cùng trong vùng tìm kiếm
                best_match = matches[-1]
                cut_position = search_start + best_match.end()
                
                # Đảm bảo có tiến bộ
                if cut_position > start:
                    return cut_position
        
        # Nếu không tìm được pattern phù hợp, tìm space cuối cùng
        for i in range(max_end - 1, start + self.max_chunk_size // 2, -1):
            if i < len(text) and text[i].isspace():
                return i + 1
        
        # Cuối cùng mới cắt cứng (nhưng tránh cắt giữa từ)
        return max_end


def get_real_openai_translation_chunked_fixed(text, target_lang, api_key, model="gpt-3.5-turbo", progress_callback=None):
    """PHIÊN BẢN ĐÃ SỬA HOÀN TOÀN - Không mất dữ liệu"""
    
    try:
        # Text ngắn - xử lý trực tiếp
        if len(text) < 2500:  # Giảm threshold để chunking sớm hơn
        ...
        # Text dài - sử dụng chunking ĐÃ SỬA
        if progress_callback:
            progress_callback(5, "🔄 Initializing enhanced chunking...")
        
        # Sử dụng chunker đã sửa
        chunker = SmartTextChunkerFixed(max_chunk_size=2500, overlap_size=100)
        chunks = chunker.chunk_text(text)
        total_chunks = len(chunks)
        
        if progress_callback:
            progress_callback(10, f"📊 Split into {total_chunks} chunks safely")
        
        print(f"🔍 TRANSLATION DEBUG: Processing {total_chunks} chunks")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                # Xử lý chunk có context
                if chunk['has_context']:
                    # Tách context và main content
                    chunk_text = chunk['text']
                    if "[MAIN_CONTENT_START]" in chunk_text and "[MAIN_CONTENT_END]" in chunk_text:
                        # Tách các phần
                        parts = chunk_text.split("[MAIN_CONTENT_START]")
                        if len(parts) == 2:
                            context_part = parts[0].replace("[CONTEXT_START]", "").replace("[CONTEXT_END]", "").strip()
                            main_part = parts[1].replace("[MAIN_CONTENT_END]", "").strip()
                            
                            system_prompt = f"""You are a professional translator. 

PREVIOUS CONTEXT (for reference only, DO NOT translate this part):
{context_part[-200:]}

Please translate ONLY the following main content to {target_language}. Ensure smooth continuation from the context above:"""
                            text_to_translate = main_part
                        else:
                    ...
                    # Chunk đầu tiên hoặc không có context
                    system_prompt = f"Translate the following text to {target_language}. Preserve formatting and meaning exactly:"
                    text_to_translate = chunk['text']
                
                print(f"🔄 Translating chunk {i+1}: {len(text_to_translate)} chars")
                ...
                # Lưu kết quả với metadata đầy đủ
                ...
                print(f"✅ Chunk {i+1} completed: {len(translated_text)} chars translated")
                ...
                # Giữ lại nội dung gốc khi lỗi
                translated_chunks.append({
                    'chunk_id': chunk['chunk_id'],
                    'translated': f"[TRANSLATION ERROR - ORIGINAL TEXT]: {chunk.get('main_content', chunk['text'])[:500]}...",
                    'original_content': chunk.get('main_content', chunk['text']),
                    'original_length': len(chunk.get('main_content', chunk['text'])),
                    'translated_length': len(chunk.get('main_content', chunk['text'])),
                    'start_pos': chunk['start_pos'],
                    'end_pos': chunk['end_pos'],
                    'error': str(e)
                })
        ...
        if progress_callback:
            progress_callback(95, "🔧 Combining results safely...")
        
        # GHÉP CHUNKS THEO CÁCH MỚI - KHÔNG MẤT DỮ LIỆU
        combined_result = combine_chunks_safely(translated_chunks, total_chunks)
        
        if progress_callback:
            progress_callback(100, "✅ Translation completed!")
        
        print(f"🎉 FINAL RESULT: {len(combined_result)} chars from {successful_chunks}/{total_chunks} chunks")
        ...
        return {
            ...
            'target_lang': target_language,
            'model_used': model,
            'confidence': 0.9 if successful_chunks == total_chunks else 0.7,
...
def combine_chunks_safely(translated_chunks: List[Dict], total_expected: int) -> str:
    """GHÉP CHUNKS AN TOÀN - KHÔNG MẤT DỮ LIỆU"""
    ...
    print(f"🔧 COMBINING: {len(sorted_chunks)} chunks in order")
    
    # PHƯƠNG PHÁP MỚI: Ghép tuần tự đơn giản
    combined_parts = []
    ...
    for i, chunk in enumerate(sorted_chunks):
        ...
        if translated_text:
            # Loại bỏ marker text nếu có
            if translated_text.startswith('[TRANSLATION ERROR'):
                # Với chunk lỗi, vẫn giữ nội dung gốc
                combined_parts.append(f"\n[Chunk {i+1} - Translation Error]\n{chunk['original_content'][:1000]}\n")
            else:
                combined_parts.append(translated_text)
            
            print(f"   ✅ Added chunk {i+1}: {len(translated_text)} chars")
        else:
            print(f"   ⚠️ Empty chunk {i+1}, skipping")
    
    # Ghép với separator nhẹ
    ...
    print(f"🎯 COMBINATION COMPLETE: {len(final_result)} chars final")
...
class ChunkedTranslationEngine:
    ...
    def _translate_single_chunk(self, chunk_text: str, target_lang: str, 
                               chunk_id: int, has_context: bool) -> Dict[str, Any]:
        """Dịch một chunk đơn lẻ"""
        ...
        try:
            ...
            if has_context and "[CONTEXT_SEPARATOR]" in chunk_text:
                context_part, main_part = chunk_text.split("[CONTEXT_SEPARATOR]", 1)
                system_prompt = f"""Bạn là chuyên gia dịch thuật chuyên nghiệp. 
                
                CONTEXT (để hiểu ngữ cảnh, KHÔNG dịch phần này):
                {context_part.strip()}
                
                Hãy dịch phần chính sau đây sang {target_lang}, giữ nguyên định dạng và ý nghĩa:"""
                text_to_translate = main_part.strip()
            else:
                system_prompt = f"Bạn là chuyên gia dịch thuật. Dịch văn bản sau sang {target_lang}, giữ nguyên định dạng và ý nghĩa:"
        ...
        if config.get('ai_translation', False):
            ...
            if text_to_translate.strip():
                ...
                if openai_key:
                    ...
                    translation_result = get_real_openai_translation_chunked(
                        text_to_translate, 
                        target_lang_code, 
                        openai_key
                    )
    ...
    with col2:
        if st.button("🚀 Start REAL AI Processing", type="primary", use_container_width=True):
            if input_text.strip():
                ...
                real_text_processing_pipeline_fixed_with_chunking(input_text, real_processing_config)
...
# Thêm 2 functions mới này vào cuối file:
def combine_chunks_safely(translated_chunks: List[Dict], total_expected: int) -> str:
...
# The `if` statement is incomplete and missing both a condition and a code block.
# Replace it with a valid if statement or remove it if not needed.

# Example fix (remove or replace as needed):
# Remove the incomplete line:
# (Just delete the line: if)

# Or, if you meant to add a condition, use:
if False:
    pass  # TODO: Add your code here

# ...existing code... __name__ == "__main__":
    main()
