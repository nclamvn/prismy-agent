...
class PodcastGenerator(BaseContentTransformer):
    ...
    def _generate_intro(self, request: TransformationRequest) -> str:
        """Generate intro phù hợp với audience using LLM"""
        ...
        if self.llm_service:
            try:
                intro = self.llm_service.enhance_podcast_content(
                    content=request.source_text[:200],
                    section_type="intro",
                    target_audience=audience.value,
                    tone="conversational" if audience != TargetAudience.PROFESSIONALS else "professional"
                )
                return intro.strip()
